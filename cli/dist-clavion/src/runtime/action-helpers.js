...
async function preloadFont(page, fontFamily) {
...
async function injectSlideOverlay(page, title, description, config, branding) {
  ...
  `;
    ...
    logo.src = '${escapeJs(config.logoUrl)}';
    logo.style.cssText = 'width:140px;height:140px;object-fit:contain;margin-bottom:28px;display:inline-block;';
...
export function createHelpers(page, collector, ctx, branding) {
    let lastX = 640;
    let lastY = 360;
    ...
    let lastSlideFile = null;
    let lastSlideEntryIndex = -1;
    let pendingTransitionMarker = null;
     ...
     * the capture loop.
     */
    async function resolvePendingTransition() {
    ...
    function emitNarrationEvent(narration) {
        const event = {
            type: 'narration',
        ...
        // Only set audioFile if present (unified audio: only first segment has the file)
        if (narration.audioFile) {
            event.audioFile = narration.audioFile;
        }
    ...
    async function moveCursorTo(toX, toY) {
        ...
        await ctx.waitForDuration(moveDurationMs);
        lastX = toX;
        lastY = toY;
    }
    ...
    function actionError(action, selector, cause) {
        const url = page.url();
        const msg = cause instanceof Error ? cause.message : String(cause);
        const err = new Error(`sw.${action}(${JSON.stringify(selector)}) failed on ${url}\n${msg}`);
        err.cause = cause;
        return err;
    }
    /** Sleep for remaining narration time after an action settles, plus buffer. */
    async function sleepRemainingNarration(startMs, durationMs) {
        const elapsedMs = ctx.currentTimeMs() - startMs;
        const remainingMs = durationMs + NARRATION_BUFFER_MS - elapsedMs;
        if (remainingMs > 0)
            await ctx.waitForDuration(remainingMs);
    }
    return {
        page,
        async scene(title, descriptionOrOptions) {
            ...
            if (slide) {
                ...
                const effectiveDurationMs = narration
                    ? Math.max(slideDurationMs, narration.durationMs + NARRATION_BUFFER_MS)
                    : slideDurationMs;
                ...
                await ctx.waitForDuration(effectiveDurationMs);
        ...
        async navigate(url, actionOpts) {
            ...
            if (narration)
                await sleepRemainingNarration(narStartMs, narration.durationMs);
        },
        async click(selector, actionOpts) {
            ...
            try {
                ...
                await ctx.waitForDuration(CURSOR_DWELL_MS);
                const locator = page.locator(selector).first();
        ...
        async narrate(text) {
            ...
            emitNarrationEvent(narration);
            await ctx.waitForDuration(narration.durationMs + NARRATION_BUFFER_MS);
        },
